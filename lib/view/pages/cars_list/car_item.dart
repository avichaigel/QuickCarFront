                  ...
                  ? Container(
                      child: GestureDetector(
                        child: Card(
                          ...
                          child: AspectRatio(
                            aspectRatio: 1,
                            child: myCar.images[0] != null
                                ? Image.network(myCar.images[0].path)
                                : Text("no image"),
                          ),
                        ),
                        onTap: () async {
                ...
                child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        Consumer<MyCurrencyService>(
                            builder: (context, state, child) {
                          return Align(
                            alignment: Alignment.topCenter,
                            child: Text(
                              "${state.getPriceInCurrency(myCar.pricePerDayUsd)} per day",
                              style: TextStyle(
  ...
  return ElevatedButton(
    ...
    onPressed: () async {
      ...
      if (car.owner == userState.getEmail()) {
        myShowDialog(
            context, "Book car failed", "You cannot book your own car");
        return;
      }
      try {
        car.placeMarks =
            await placemarkFromCoordinates(car.latitude, car.longitude);
      } catch (e) {
        print("no location found");
      }
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (BuildContext context) => ReservationDetails(car)));
    },
  );
}
